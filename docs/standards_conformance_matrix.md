...
| 3GPP TR 36.777 v15.0.0, Table B-2 | UMa-AV LOS and high-altitude NLOS terms | `uma_av_path_loss_db`; 2 GHz, UAV altitude 50–300 m | Elevation-dependent LOS probability is a disclosed scenario-generator choice | 1 km LOS at 100 m altitude, 0.1 dB tolerance | Source-derived deterministic vector. LLSim5G is not used here because its A2G implementation cites Khawaja et al. (2019), not TR 36.777. |
...
The frozen external file is generated by executing pinned upstream commits:

```powershell
tmp\venv-opentnn\Scripts\python.exe scripts\generate_external_reference_outputs.py --llsim-root tmp/vendor/LLSim5G --openntn-root tmp/vendor/OpenNTN
python scripts\run_physical_conformance.py --external standards\external_reference_outputs.csv
```

`standards/external_tools.json` records the repositories, commits, and OpenNTN/Sionna environment. Missing values for the three applicable OpenNTN/LLSim5G vectors remain an explicit release blocker. The TR 36.777 vector is explicitly marked `not_applicable` for external comparison; it still must pass its source-derived 0.1 dB reference test.
